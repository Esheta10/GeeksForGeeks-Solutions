...
class Solution {
  public:
    ...
    // Step 2: Link the current node to the node after the next node
    Node* temp = del_node->next; // Store the next node temporarily
    del_node->next = del_node->next->next;

    // Step 3: Delete the next node (originally the next node after del_node)
    delete temp;
    }
};
