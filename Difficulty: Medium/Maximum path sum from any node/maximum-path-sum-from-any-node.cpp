...
class Solution {
  ...
  public:
    ...
    int findMaxPath(Node* root) {
        ...
        int rightSum = max(0, findMaxPath(root->right)); // Ignore negative sums

        // Update the global maximum path sum considering root as the turning point
        maxSum = max(maxSum, root->data + leftSum + rightSum);

        // Return max path sum starting from the current node
